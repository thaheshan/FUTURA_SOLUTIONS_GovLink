...
import { SETTING_CHANNEL, SETTING_KEYS } from '../constants';
...
export class SettingService {
  ...
  async update(key: string, data: SettingUpdatePayload): Promise<SettingDto> {
    ...
    if (key === SETTING_KEYS.FREE_SUBSCRIPTION_ENABLED && data.value === false) {
      await this.performerModel.updateMany({}, { $set: { isFreeSubscription: false } });
    }
    if (key === SETTING_KEYS.FREE_SUBSCRIPTION_DURATION) {
