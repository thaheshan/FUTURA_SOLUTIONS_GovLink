...
import {
    ...
    REACTION_TYPE,
    REACTION
} from 'src/modules/reaction/constants';
...
interface ReactComment extends QueueEvent {
    data: {
        objectId: string;
        objectType: string;
        action: string;
    ...
    public async handleReactComment(event: ReactComment) {
        ...
        if (event.eventName === EVENT.CREATED) {
            if (comment) {
                ...
                await this.performerService.updateLikeStat(
                    comment.createdBy,
                    1
                );
            }
