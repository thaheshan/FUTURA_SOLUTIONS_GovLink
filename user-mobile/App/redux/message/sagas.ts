...
const conversationSagas = [
  {
    ...
    *worker(data: IReduxAction<Record<string, string>>) {
      try {
        ...
        yield put(
          getConversationsSuccess({ ...resp.data, isMore: data.payload.isMore })
        );
      } catch (e) {
        // TODO - alert error
        console.log({ e });
        ...
        yield put(getConversationsFail(JSON.stringify(error)));
      }
    },
  },
  {
    ...
    *worker(data: IReduxAction<Record<string, string>>) {
      try {
        ...
        if (conversationId) {
          if (conversationMapping[conversationId]) {
            ...
            const readAllMessages =
              yield messageService.readAllInConversation(conversationId);
            if (readAllMessages) {
        ...
        console.error({ e, setActiveConversation });
  ...
  },
  {
    ...
    *worker(data: IReduxAction<Record<string, any>>) {
      try {
        ...
        console.error({ e, loadMoreMessages });
        message.error("Error occurred, please try again later");
        // console.log('err', e);
      }
    },
